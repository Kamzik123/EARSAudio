...
/* Encode options. Zero-initialize and override only what you need. */
typedef struct {
    /* Frames per SNS block. 0 = default (256, one big block when possible).
     * The game's streamed files use 26 frames per block; set this to 26 to
     * match that shape. Legal range: 1..524287. */
...
/* Same as ears_encode_memory but with options (pass NULL for defaults). */
EARS_API ears_status ears_encode_memory_ex(const int16_t *pcm, size_t samples,
                                           int channels, int sample_rate,
